...
from app.utils.update_output_draft_status import update_output_draft_status
from app.core.db import get_session
from app.models import OutputDetail, Source
...
async def read_output_details(
    ...
    stmt = (
        select(
            ...
            OutputDetail.title.label("title"),
            OutputDetail.created_at.label("created_at"),
            OutputDetail.file_path.label("file_path"),
            OutputDetail.file_size.label("file_size"),
            OutputDetail.file_type.label("file_type"),
            OutputDetail.converted_file_path.label("converted_file_path"),
            OutputDetail.is_out_of_sync.label("is_out_of_sync"),
            ...
            func.coalesce(
                ...
                'NA'
                ).label("reporting_effort"),
            func.coalesce(
                OutputDetail.database_release_name,
                'NA'
                ).label("database_release"),
            func.coalesce(
            ...
            else_="").label("docs_shared_as"),
            func.coalesce(
                func.avals(OutputDetail.tags),
                []
            ).label("reporting_effort_tags"),
            func.coalesce(func.akeys(OutputDetail.tags), []).label("tag_ids")
        )
    )
...
async def sync_output_details(
    ...
    try:
        provided_ids = list(set(output_ids.output_detail_ids))
        # sync_results = await update_multiple_versions_to_s3(session, provided_ids)
        # synced_ids = [r["id"] for r in sync_results.get("details", []) if r.get("success")]
        stmt = select(OutputDetail).where(OutputDetail.id.in_(provided_ids))
        result = await session.execute(stmt)
        ...
        # Update is_out_of_sync to False for all provided IDs
        for od in output_details:
            try:
                od.is_out_of_sync = False
                session.add(od)
                synced_identifiers.append(od.identifier)
                synced_ids.append(od.id)
            except Exception as e:
                failed_identifiers.append(od.identifier)
        
        # Commit changes
        await session.commit()
        get_user_to_output_mapping.delay(list(synced_ids))
        # synced_identifiers = [r["identifier"] for r in sync_results.get("details", []) if r.get("success")]
        # failed_identifiers = [r["identifier"] for r in sync_results.get("details", []) if not r.get("success")]
        response_string = ""
        if synced_identifiers:
            response_string += f"Successfully synced OutputDetails: {', '.join(synced_identifiers)}."
        if failed_identifiers:
            response_string += f"Failed to sync OutputDetails: {', '.join(failed_identifiers)}."
        return JSONResponse(
            status_code=202,
            content={"detail": response_string}
        )
