...
async def apply_filter(
    ...
    if filters.database_release_id__in and not filters.scan_recursively:
        filters.scan_recursively = True
        ...
        if filters.advanced_search:
            stmt = stmt.where(
                BooleanSearchFilter.build_array_filter(
                    filters.tag_name, func.avals(OutputDetail.tags),
                    )
            )
        else:
            stmt = stmt.where(
                literal(filters.tag_name).ilike(any_(func.avals(OutputDetail.tags)))
            )

    if filters.tag_id__in:
        stmt = stmt.where(
            (
                func.coalesce(func.akeys(OutputDetail.tags), []).op("&&")
