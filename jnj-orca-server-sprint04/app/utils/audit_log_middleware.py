...
from app.models import AuditLog, DistributionList, OutputDetail, ReportingEffortTag, DatabaseReleaseTag, ReportingEffortTagDistributionListLink, OutputDetailTagLink, DatabaseReleaseTagDistributionListLink, OutputDetailDatabaseReleaseTagLink
...
async def get_m2m_data_for_tags(db, model, resource_id: int) -> dict:
    """Return many-to-many related data for ReportingEffortTag and DatabaseReleaseTag."""
    m2m_data = {}

    if model == ReportingEffortTag:
        # Output details
        result = await db.execute(
            select(OutputDetail.id)
            .join(OutputDetailTagLink)
            .where(OutputDetailTagLink.tag_id == resource_id)
        )
        m2m_data["output_details"] = [row[0] for row in result.fetchall()]

        # Distribution lists
        result = await db.execute(
            select(DistributionList.id)
            .join(ReportingEffortTagDistributionListLink)
            .where(ReportingEffortTagDistributionListLink.reporting_efforttag_id == resource_id)
        )
        m2m_data["distribution_lists"] = [row[0] for row in result.fetchall()]

    elif model == DatabaseReleaseTag:
        # Output details
        result = await db.execute(
            select(OutputDetail.id,OutputDetail.orca_version,OutputDetail.space_version,OutputDetail.source_name)
            .join(OutputDetailDatabaseReleaseTagLink)
            .where(OutputDetailDatabaseReleaseTagLink.database_release_tag_id == resource_id)
        )
        # m2m_data["output_details"] = [row[0] for row in result.fetchall()]
        m2m_data["output_details"] = [{"id": row[0], "version": row[1],"source_name":row[3]} for row in result.fetchall()]
...
async def audit_middleware(request: Request, call_next):
    ...
    if resource_type == "distribution-lists":
        ...
        resource_ids = payload.get("output_detail_ids", [])
